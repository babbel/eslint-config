...
- ESLint version `8.56.0` is now the minimum supported version, but version `9.x` cannot be supported yet due to a dependency on the `eslint-config-airbnb*` packages and its strict requirement for the flat config format
